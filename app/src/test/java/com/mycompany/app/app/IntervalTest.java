...
public class IntervalTest {
	...
	public void testHashCode() {
		...
		Interval anotherTwoTone = new Interval(2, 0);
		Interval oneToneTwoHalf = new Interval(1, 2);
		Interval fiveHalf = new Interval(0, 5);
		assertEquals("the object are the same so hash code should be equals", twoTone.hashCode(), twoTone.hashCode());
		assertEquals("the object are composed of the same intervals and so has code should be equals", twoTone.hashCode(), anotherTwoTone.hashCode());
		assertEquals("the object are composed by different parts but the intervals are equal so has code should be equals", twoTone.hashCode(), oneToneTwoHalf.hashCode());
		assertNotEquals("the object are composed by different parts and the intervals are not equal so has code should not be equals", fiveHalf.hashCode(),
				oneToneTwoHalf.hashCode());
	}
